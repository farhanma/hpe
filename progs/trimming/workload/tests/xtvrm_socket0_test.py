...
#
# xtvrm_socket0_test.py - a wrapper script used in conjunction with xtvrm_socket0_test.ini
#                       to execute the standard Cray xtcpudgemm diagnostic
#
# author: Pete Halseth
#
# The purpose of xtvrm_socket0_test.py is to provide the means to include 
# the standard Cray xtcpudgemm diagnostic as a component test in a list
# of tests grouped together in xtsystest.ini
#
# Usage:
# -------
# 1. To run as a standalone test: ./xtvrm_socket0_test.py <options> 
#    To see the available options, type: ./xtvrm_socket0_test.py -h 
#
# 2. To use within a script, just include the "import xtvrm_socket0_test" statement at 
# the top of the script
# 
################################################################################
##@package tests.xtvrm_socket0_test
...
class XTVrmScreen_Socket0(XTVrmScreen):
    ...
    if test is not None:
        ...
        if test.initialize(test_options):
            ...
            if not validation_errors:
                ...
                test.generate_power_csv(test.log_file_name,results_output_filename.replace("socket_number","socket0"),["Passed","passed","Failed","failure"],[r'c[*0-9]+\-[*0-9]+c[*0-2]s[0-1]?[0-9]'],test.get_component_option("bin_sort_column_label"),0)
                ...
                if watts_output_filename:
                    ...
                    test.generate_power_csv(test.log_file_name,watts_output_filename.replace("socket_number","socket0"),["Passed","passed","Failed","failure"],[r'c[*0-9]+\-[*0-9]+c[*0-2]s[0-1]?[0-9]'],test.get_component_option("watts_sort_column_label"),0)
                time.sleep(10)
