...
from xthpl_multinode_test import XTHplMultiNodeTest 
                        ...
                        if not residual_key == longest_list_key_name:
                            ...
                            for index,nid in enumerate(residuals[residual_key]):
                                ...
                                if "c" in nid:
                                    ...
                                    cnames = []
                                    if "-" in nid or "," in nid:
                                        nid_list = test.sysconfig.expand_node_list(nid)
                                        if nid_list:
                                            for nid in nid_list:
                                                cnames.append(test.get_cname_from_nid_using_cnames_dictionary(int(nid)))
                                        else:
                                            cnames.append(nid)
                                    else:
                                        cnames.append(test.get_cname_from_nid_using_cnames_dictionary(int(nid)))
                                    if cnames:
                                        failed_residual_cnames.extend(cnames)
                                    else:
